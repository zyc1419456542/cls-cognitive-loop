...
## v3.2 更新记录（2026-09-18）

v3.1（8 月 28 日）之后的三周里，系统新增了以下机制。全部在生产会话中验证过。

**认知分层**。PostToolUse 给每条工具轨迹打层标签：reflex（读文件、列目录这类反射动作）、rhythm（常规写入）、strategy（新方案、架构改动）、human（人类消息）。标签从现有日志推导，单次 5.2 毫秒，不新增监控进程。层分布数据是后续按层配比训练数据的原料。

**stance 四档**。会话级工作档位：farming（日常）、skirmish（卡住，强制换策略）、teamfight（多窗口协同）、retreat（诊断保护）。唯一写入口是 cog-context 的 set-stance（CAS 原子写），带 TTL，过期自动回 farming。retreat 档只提醒不拦截——保护调试现场。

**开团前 CD 清点**。Stop.ps1 在会话收尾时清点未处理债务：commit 后没有留下决策声明、修复超过 3 轮没有记 incident、交付没有走检查门。只亮牌，不拦截，一个信封，2 秒内完成。

**声明即检索**。cog-step-declare 的 label 现在同时是检索键：声明的类别和标签直接命中知识卡（词面匹配 473 毫秒），声明完成的时刻卡片已经送到。消除了"先声明、再等注入"的两段延迟。

**知识卡 v2.1**。pitfall 卡改为错误写法与正确写法成对出现——对比才有信息量；highlight 卡（纯正确知识）单独一列。解析器对围栏、嵌套、裸换行三种格式容错。

**被动事实扫描**。transcript_fact_scan 从会话缓存提取四类事实：空转、修复循环、工具单调、停滞。external_anchor 用 git 记录和进程表这类物理证据，对质"工具全部返回成功但状态没变"的静默失败。

**错误解药表**。PostToolUseFailure 挂载 29 条解药，覆盖 11 类高频报错。报错关键词先查表，查不到再调模型。

**dsh 侧 cls-inject 升到 v0.4**。漂移检测改用本地微调模型做纯推理；跨窗口卡片注入加身份与年龄双重校验（声明 30 分钟内有效）。
...
| 声明协议全局死锁，写入全被拦 | MCP 包装层函数签名丢失形参，叠加读写两端字段名不一致 | 门禁对状态文件自身留豁免口，借系统函数先补合规声明，再修根因 |
| 中文 PowerShell 脚本整文件解析失败，硬闸静默空转数日 | UTF-8 无 BOM 被 PowerShell 5 按 ANSI 读取，乱码吞掉引号 | 中文脚本一律带 BOM；hook 报"某行解析错误"先查编码 |
| 孤儿进程收割器每 2 分钟杀一次另一运行时的 MCP 子进程，最终拖死其主进程 | 判定规则只认 claude 为合法父进程 | 合法父进程白名单双层化（claude + 另一运行时）；新起常驻进程先查收割器火力圈 |
