...
$MaxDepth={MAX_RECURSION_DEPTH};$TTL={COG_STEP_TTL}
...
if($tool_name -in @("Write","Edit")){
  ...
  if($t -notmatch "cog_step\.json$"){
    ...
    try{$c=Get-Content $CogStepFile -Raw|ConvertFrom-Json;$a=[int]((Get-Date)-[datetime]::Parse($c.declared_at)).TotalSeconds
      if($a -gt $TTL){Log "BLOCK" "COG_EXPIRED" "age=$a";Write-Output (@{permissionDecision="deny";permissionDecisionReason="[COG_STEP] Expired ($a>$TTL s). Re-declare."}|ConvertTo-Json -Compress);exit 0}
...
if($tool_name -in @("Write","Edit") -and $t){$n=[IO.Path]::GetFullPath($t);foreach($p in $ProtectedPaths){if($n -eq [IO.Path]::GetFullPath($p)){Log "BLOCK" "WRITE_PROTECT" $t;Write-Output (@{permissionDecision="deny";permissionDecisionReason="[FUSE:WRITE_PROTECT] $t"}|ConvertTo-Json -Compress);exit 0}}}
# CHECK 3: RECURSION_LIMIT
$d=0;if(Test-Path $RecursionFile){try{$d=[int](Get-Content $RecursionFile -Raw|ConvertFrom-Json).depth}catch{}}$d++;if($d -gt $MaxDepth){Log "BLOCK" "RECURSION" "depth=$d";Write-Output (@{permissionDecision="deny";permissionDecisionReason="[FUSE:RECURSION] $d>$MaxDepth"}|ConvertTo-Json -Compress);exit 0}
